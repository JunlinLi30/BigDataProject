...
public class UnitSum {
    public static class PassMapper extends Mapper<Object, Text, Text, DoubleWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            //key = toId
            //value = subPr
            //line = toId\tsubPr (MP默认之间用\t间隔)
           String[] pageSubrank = value.toString().trim().split("\t");//idSubpr
            double subRank = Double.parseDouble(pageSubrank[1]);
            context.write(new Text(pageSubrank[0]), new DoubleWritable(subRank));//读进来(拆分好数据---所以mapper是必需)马上写出
        }
    }
